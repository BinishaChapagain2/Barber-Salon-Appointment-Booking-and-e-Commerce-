...
class BuyNowController extends Controller
{
    ...
    public function checkout()
    {
        ...
        if (!$buyNowData) {
            return redirect()->route('homepage')->with('success', 'No product selected for purchase.');
        }
    ...
    public function placeOrder(Request $request)
    {
        ...
        if ($buyNowData['qty'] == 0 || $buyNowData['qty'] > Product::find($buyNowData['product_id'])->stock) {
            return redirect()->route('homepage')->with('success', 'Product is out of stock');
        }
    ...
    public function store(Request $request, $productid, $qty)
    {
        ...
        if (!$product || $request->qty > $product->stock) {
            return redirect('homepage')->with('success', 'Product is out of stock');
        }
