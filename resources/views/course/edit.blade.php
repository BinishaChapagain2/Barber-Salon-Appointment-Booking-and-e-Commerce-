...
@section('title')
    Update Course | Gentleman BarberShop
@endsection

@section('headerofadmin')
    <div class="w-full px-4 py-4 bg-[#051923] shadow-md text-white">
        <i class="bx bx-menu menu-icon"></i>
        <span class="font-medium logo-name lg:text-lg">Update New Course</span>
    </div>
@endsection
...
@section('content')
    <div class="flex flex-col flex-1 w-full h-auto bg-gray-300">
        ...
        <div class="w-full p-8 bg-white rounded-lg shadow-md">
            <h2 class="mb-6 text-xl font-bold">Edit New Course</h2>
            <form action="{{ route('course.update', $course->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="discounted_price" id="discounted_price"
                        class="block w-full p-3 mt-1 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Enter discounted price (optional)" value="{{ $course->discounted_price }}">
                ...
                <div>
                    ...
                    @enderror
                    <p>Current Picture:</p>
                    <img src="{{ asset('images/courses/' . $course->photopath) }}" alt="" class="w-44">
                </div>
