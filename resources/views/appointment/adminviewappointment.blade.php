...
@section('headerofadmin')
    <div class="flex justify-between w-full px-4 py-4 bg-[#051923] shadow-md text-white">
        <div class="flex items-center space-x-2 left">
            ...
            <span class="text-sm font-medium logo-name sm:text-base md:text-lg lg:text-xl">View Appointments</span>
        </div>
...
@section('content')
                        ...
                        <tr class="transition duration-300 border-b bg-gray-50 hover:bg-blue-100">
                            ...
                            <td class="px-4 py-2">
                                <div class="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2">
                                    <a href="{{ route('appointment.status', ['id' => $appointment->id, 'status' => 'pending']) }}"
                                    ...
                                    <a href="{{ route('appointment.status', ['id' => $appointment->id, 'status' => 'approved']) }}"
                                        class="w-full px-4 py-2 text-xs font-bold text-white bg-blue-500 rounded hover:bg-blue-700 sm:w-auto sm:text-sm md:text-base lg:text-sm">Approved</a>
    ...
    <div class="fixed inset-0 items-center justify-center hidden bg-gray-600 bg-opacity-50 backdrop-blur-sm" id="popup">
        <form action="{{ route('appointment.delete') }}" class="px-8 py-6 text-center bg-white rounded-lg shadow-lg"
            method="POST">
            @csrf
...
<script>
    function showpopup(appointment_id) {
